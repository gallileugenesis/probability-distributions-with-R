...
# probabilidade de sucesso é 50%. Ao passar 10 clientes em nossa loja, qual a 
# probabilidade de realizarmos 2 vendas? Ou seja, queremos encontrar a 
# probabilidade de dois sucessos, em dez tentativas. Cuja probabilidade 
# de sucesso # em cada tentativa é 50%
...
# A função a seguir gera quantidades aleatórias de sucesso oriundos de 
# uma quantidade (size) de tentativas dada a probabilidade (prob) de sucesso.
# É útil para realizar experimentos. Podemos simular qual a frequencia 
# esperada de vendas a cada dez clientes ? Ainda mantendo a probabilidade de 
# sucesso (cliente comprar) de 50%
...
# (probabilidade de nenhum cliente comprar) + 
# (probabilidade de um cliente comprar) +
# probabilidade de dois cliente comprarem)
# Formalizando: P(X<=2) = P(X=0) + P(X=1) + P(X=2)

pbinom(q = 2,size = 10, prob = 0.5)

# A probabilidade de que até dois clientes comprem ao entrarem dez clientes, 
# é de 5,48%
...
# Veja como as probabilidades vão diminuindo. A probabilidade de sucesso de 50% 
...
# probabilidade do primeiro sucesso ocorrer na primeira tentativa OU na segunda 
...
# Qual a probabilidade de ter que entrar 8 clientes até que a segunda 
# venda ocorra?
...
# Exemplo: Uma loja recebe em média, 6 (λ) clientes por minuto. Qual a 
...
# Podemos utilizar a mesma funcao para obter a probabilidade de entrar 
# um cliente, dois clientes... quinze clientes

va_poison <- dpois(x = 1:15, lambda = 6)
plot(va_poison)

# Observe que os valores se distribuiem simetricamente en tormo de seis, 
# use acontece porque o paramentro lambda é a média (e também o desvio padrão) 
# da distribuição de Poisson

# Também podemos obter a probabilidade acumulada de até 5 clientes entrarem 
# na loja em um minuto
...
# Exemplo: Suponha que a distribuição dos salários dos funcionários de 
# uma empresa sigam uma distribuição normal com média μ=2.500 e 
# desvio padrão σ= 170. Ao selecionar aleatoriamente um indivíduo dessa 
...
# Ao aplicar o comando na variável va_normal que acabmos de criar, ela ficará 
...
# empresa sigam uma distribuição #normal com média μ=2.500 e desvio padrão 
# σ= 170. Ao selecionar aleatoriamente um indivíduo dessa população, qual a 
# probabilidade de ter #s alário acima de 2.600 ?
